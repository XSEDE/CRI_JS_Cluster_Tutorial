...
for user in $(awk '/train/ {print $1'} tg_passwords.txt); do
  source openrc-${user}.sh
  server_public_ip=$(openstack server list -f value -c Networks --name ${user}${public_server_suffix} | sed 's/.*, //')
  echo ${user} ${OS_USERNAME} ${server_public_ip}
  openstack server remove floating ip ${user}${public_server_suffix} ${server_public_ip}
  openstack floating ip delete ${server_public_ip}
  for i in `seq 0 $((${#os_components[*]} - 1))`; do # I apologize.
    echo "Removing ${os_components[${i}]} for ${user}:"
    openstack ${os_components[${i}]} list -f value -c Name | grep -E "${OS_USERNAME}|${user}"
    particulars=$(openstack ${os_components[${i}]} list -f value -c ID -c Name | grep -E "${user}|${OS_USERNAME}" | cut -f 1 -d' ' | tr '\n' ' ') # this should grab head & computes
